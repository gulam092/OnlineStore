...
class ReportController extends Controller
{
  ...
  return view('admin.report.today_order',compact('order'));
    ...
    {
    $today = date('d-m-y');
  			$order = DB::table('orders')->where('status',3)
    ...
    {
    	$year  =$request->year;
    	$total = DB::table('orders')->where('status',3)
    	            ->where('year',$year)->sum('total');
    	$order = DB::table('orders')->where('status',3)
    	                            ->where('year',$year)->get();
