                ...
                <ul class="cart_list">
                    ...
                    <li class="cart_item clearfix text-center">
                        ...
                        <div class="cart_item_info d-flex flex-md-row flex-column justify-content-between">
                         ...
                         <img src="{{ asset($row->options->image)}}" alt=""style="width: 70px; width: 70px;">
                     </div>
                 </div>

 
                            <div class="cart_item_name cart_info_col">
                                <div class="cart_item_title"> <b>
                                Name</b></div>
                                <div class="cart_item_text">
                                    {{ $row->name  }}</div>
                            </div>
              ...
              @endif
                ...
                @if(Session::has('coupon'))
                 <li class="list-group-item">Total : 
                    ...
                    ${{ Session::get('coupon')['balance'] + $charge + $vat }} </span> 
                 </li>
                 @else
                <li class="list-group-item">Total : 
...
<form action="{{ route('stripe.charge') }}" method="post" id="payment-form">
      ...
      <input type="text" name="shipping" value="{{ $charge }} ">
       <input type="text" name="vat" value="{{ $vat }} ">
       <input type="text" name="total" value="{{ Cart::Subtotal() + $charge + $vat }} ">

  <input type="text" name="ship_name" value="{{ $data['name'] }} ">
  <input type="text" name="ship_phone" value="{{ $data['phone'] }} ">
  <input type="text" name="ship_email" value="{{ $data['email'] }} ">
  <input type="text" name="ship_address" value="{{ $data['address'] }} ">
  <input type="text" name="ship_city" value="{{ $data['city'] }} ">
  <input type="text" name="payment_type" value="{{ $data['payment'] }} ">
...
var stripe = Stripe('pk_test_DPKl5WxBzKMVmXnFEQBwkabl00DY34TgTc');
